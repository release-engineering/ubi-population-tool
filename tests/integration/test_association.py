...
def test_modulemd_defaults_unit_copy():
    ...
    assert mod_name == "httpd", "Expected module: httpd, found module: {}".format(
        mod_name
    )
    assert (
        "common [d]" in mod_profile
...
def test_ubipop_can_associate_packages():
    ...
    def assert_pkgs(pkgs_included, pkgs_found):
        ...
        for pkg in pkgs_found:
            assert (
                pkg in pkgs_included
            ), '"{}" is on repository but not in whitelist.'.format(pkg)
...
def test_ubipop_can_associate_modules():
    ...
    def assert_mds(mds_included, mds_found):
        ...
        for md in mds_found:
            assert (
                md in mds_included
            ), '"{}" is on repository but not in whitelist.'.format(md)

        for md in mds_included:
            assert (
                md in mds_found
            ), '"{}" is whitelisted but not in the repository.'.format(md)
    ...
    for repo in rpm_repos:
        ...
        for md_name in mds_included:
            ...
            profiles = [
                md.profiles for md in cfg.modules.whitelist if md.name == md_name
            ][0]
...
def test_add_packages_multiple_arch():
    ...
    repo_rpms = query_repo_rpms(None, "ubi", repo_url)
    assert all(
        "glibc" not in rpm for rpm in repo_rpms
...
def test_artifact_copy_if_profiles_not_speficied():
    ...
    expected1 = [
        ...
        "httpd-tools-2.4.6-88.el7.x86_64.rpm",
        "jss-4.4.4-3.el7.x86_64.rpm",
        "nuxwdog-1.0.3-8.el7.x86_64.rpm",
    ]
    for rpm in repo_rpms:
        assert all(
            new_rpm != rpm for new_rpm in expected1
    ...
    assert all(
        expected2 != rpm for repo in repo_rpms
    ), "Unexpected rpm in srpm content set."
    unexpected = "jss-debuginfo-4.5.0-1.module+el8+2656+97b10827.x86_64.rpm"
    assert all(
        unexpected != rpm for repo in repo_rpms
    ), "Unexpected rpm in srpm content set."

    srpm_rpm_repos = get_repos_from_cs(
    ...
    repo_rpms = query_repo_rpms(None, "ubi", repo_url)
    expected3 = [
        "httpd-2.4.6-88.el7.src.rpm",
        "jss-4.4.4-3.el7.src.rpm",
        "nuxwdog-1.0.3-8.el7.src.rpm",
    ]
    for rpm in repo_rpms:
        assert all(
            new_rpm != rpm for new_rpm in expected3
    ...
    repo_rpms = query_repo_rpms(None, "ubi", repo_url)
    assert (
        expected3[0] in repo_rpms
    ), "Package in module profile has not been added to ubi repo."
    assert (
        expected3[1] in repo_rpms
    ), "Package in module artifacts has not been added to ubi repo."
    assert (
        expected3[2] in repo_rpms
    ), "Package in module artifacts, which is also excluded has not been added to ubi repo."
