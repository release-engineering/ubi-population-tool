...
def fixture_ubi_repo_set():
    yield UbiRepoSet(
        RepoSet(
            [get_test_repo(repo_id="foo-rpms", ubi_config_version="7")],
            [get_test_repo(repo_id="foo-source", ubi_config_version="7")],
            [get_test_repo(repo_id="foo-debug", ubi_config_version="7")],
        ),
        RepoSet(
            get_test_repo(repo_id="ubi-foo-rpms", ubi_config_version="7"),
            get_test_repo(repo_id="ubi-foo-source", ubi_config_version="7"),
            get_test_repo(repo_id="ubi-foo-debug", ubi_config_version="7"),
        ),
    )
...
def test_skip_outdated_dot_repos(
    mocked_search_repo_by_cs, mocked_ubipop_runner, caplog
):
...
def test_raise_config_missing(mocked_search_repo_by_cs, mocked_ubipop_runner, caplog):
    ...
    assert (
        "Config file ubiconf_golang.yaml missing from 7 and default 7 branches"
        in caplog.text
    )
...
def test_get_population_sources_repo_note(mocked_search_repo_by_id):
    ...
    mocked_search_repo_by_id.side_effect = [
        [
            get_test_repo(
                repo_id="src_1",
    ...
    repos = ubipop._get_population_sources(
        repo, None
...
def test_get_packages_from_module_by_name(mock_ubipop_runner):
    ...
    rpms, debug_rpms = mock_ubipop_runner.get_packages_from_module(
...
def test_packages_names_by_profiles(mock_ubipop_runner):
    ...
    pkg_names = mock_ubipop_runner.get_packages_names_by_profiles(
        profiles_from_ubiconfig, modules
    )
...
@pytest.mark.parametrize(
    "n, expected_versions_modules",
    [(1, {3: 2}), (2, {2: 3, 3: 2}), (3, {1: 1, 2: 3, 3: 2})],
)
def test_keep_n_latest_module_duplicate_versions(
    mock_ubipop_runner, n, expected_versions_modules
):
...
@pytest.mark.parametrize(
    "n, expected_versions", [(1, [3]), (2, [2, 3]), (3, [1, 2, 3])]
)
def test_keep_n_latest_modules_no_duplicate_version(
    mock_ubipop_runner, n, expected_versions
):
    modules = [
...
def test_log_pulp_action(capsys, set_logging, mock_ubipop_runner):
    ...
    associations = [
        AssociateActionModules(
            [get_test_mod(name="test_assoc", src_repo_id=src_repo.repo_id)],
            dst_repo,
            [src_repo],
        )
    ]
    unassociations = [
        UnassociateActionModules([get_test_mod(name="test_unassoc")], dst_repo)
    ]
    ...
    assert err == ""
    assert (
        assoc_line.strip() == "Would associate test_assoc:::: from test_src to test_dst"
    )
...
def test_filter_pkgs_from_modules(mock_ubipop_runner):
    ...
    assert (
        mock_ubipop_runner.repos.pkgs_from_modules["test5.30"][0].filename
        == "tomcatjss-7.3.6-1.el8+1944+b6c8e16f.noarch.rpm"
    )
